...
void HybridUIApp::Draw(const GameTimer& gt)
{
    ...
    if (IsUseSharedUI)
    {
            ...
            currentFrameResource->SecondRenderFenceValue))
        {
            ...
            UIPath->Render(cmdList, MainWindow);
    ...
    {
        UIPath->Render(primeCmdList, MainWindow);
    }
...
void HybridUIApp::InitRenderPaths()
{
    ...
    UIPath = std::make_shared<UILayer>(primeDevice, MainWindow->GetWindowHandle(), MainWindow);
...
{    
    ...
    if ((timer.TotalTime() - timeElapsed) >= 1.0f)
    {
        ...
        if (writeStaticticCount >= StatisticStepSecondsCount)
        {
            ...
            {
                Flush();
                UIPath->ChangeDevice(secondDevice, MainWindow);
                IsUseSharedUI = true;
            }
            else
            {
    ...
    case WM_KEYDOWN:
        {
            ...
            if (keycode == (VK_SPACE) && keyboard.KeyIsPressed(VK_SPACE))
            {
                ...
                Flush();
                UIPath->ChangeDevice(IsUseSharedUI ? secondDevice : primeDevice, MainWindow);
            }
