...
void UILayer::DrawBottomPanel()
{
    ...
    for (size_t i = 0; i < 15; ++i) {
        ...
        ImGui::ImageButton((void*)srvPircutre.GetGPUHandle().ptr, ImVec2(iconSize, iconSize), ImVec2(0, 0), ImVec2(1, 1), 0); // Замени на иконки
        //ApplyGradientAndStarsEffect(ImVec2(iconSize, iconSize), baseColors[currentColorIndex], endColors[currentColorIndex]);
        currentColorIndex++;
    ...
    for (size_t i = 0; i < 15; ++i) {
        ImGui::PushID(static_cast<int>(i));
        ImGui::ImageButton((void*)srvPircutre.GetGPUHandle().ptr, ImVec2(iconSize, iconSize), ImVec2(0, 0), ImVec2(1, 1), 0);
        //ApplyGradientAndStarsEffect(ImVec2(iconSize, iconSize), baseColors[currentColorIndex], endColors[currentColorIndex]);
        currentColorIndex++;
        if (ImGui::IsItemHovered()) ImGui::SetTooltip("Способность %zu", i + 1);
        if (i < 14) ImGui::SameLine();
...
void UILayer::Update()
{
    auto cmd = device->GetCommandQueue(GQueueType::Compute)->GetCommandList();

    GradientNoiseCB par;
    par.ColorStart = XMFLOAT4(0.8f, 0.2f, 0.1f, 1.0f);  // Красный
    par.ColorEnd = XMFLOAT4(0.1f, 0.3f, 0.9f, 1.0f);    // Синий

    float hue = fmod(totalTime * 0.1f, 1.0f);
    par.ColorStart = XMFLOAT4(
        fabs(sin(hue * 3.14f)),
        fabs(cos(hue * 1.5f)),
        fabs(sin(hue * 2.0f + 1.0f)),
        1.0f
    );
    par.ColorEnd = XMFLOAT4(
        fabs(cos(hue * 2.0f)),
        fabs(sin(hue * 3.0f + 0.5f)),
        fabs(cos(hue * 1.2f)),
        1.0f
    );

    par.TextureSize = XMFLOAT2(256.0f, 256.0f);
    par.Time = totalTime;
    par.ColorShiftSpeed = 0.5f; // Скорость перехода
    par.NoiseIntensity = 0.1f; // Слабая текстура

    cmd->TransitionBarrier(gradientTex.GetD3D12Resource(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    cmd->FlushResourceBarriers();

    cmd->SetDescriptorsHeap(&uavPicture);
    cmd->SetPipelineState(gradientPSO);
    
   // cmdList->SetRootConstantBufferView(0, params);
    cmd->SetRoot32BitConstants(0, sizeof(GradientNoiseCB) / sizeof(float), &par, 0);
    cmd->SetRootDescriptorTable(1, &uavPicture);
    ...
    cmd->TransitionBarrier(gradientTex.GetD3D12Resource(), D3D12_RESOURCE_STATE_COMMON);
    cmd->FlushResourceBarriers();
...
void UILayer::InitializeGradientResources()
{
    ...
    gradientTex = GTexture(device, gradientText, L"Gradied tex");
    ...
    gradientTex.CreateUnorderedAccessView(&uavDesc, &uavPicture);
    ...
    gradientTex.CreateShaderResourceView(&srvDesc, &srvPircutre);
    ...
    gradientShader = GShader(L"Shaders\\GradientCompute.hlsl", ComputeShader, nullptr, "CSMain", "cs_5_1");
    gradientShader.LoadAndCompile();

    gradientPSO.SetRootSignature(gradientSignature);
    gradientPSO.SetShader(&gradientShader);
    gradientPSO.Initialize(device);
}
...
void UILayer::Render(const std::shared_ptr<GCommandList>& cmdList, std::shared_ptr<GCrossAdapterResource> & res)
{
    auto start = std::chrono::high_resolution_clock::now();

    cmdList->SetDescriptorsHeap(&srvMemory);
    cmdList->SetDescriptorsHeap(&srvPircutre);
    cmdList->SetDescriptorsHeap(&uavPicture);
