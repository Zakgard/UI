...
{
    srvUavMemory = device->AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 300);
...
void UILayer::DrawRightBar()
{
    ...
    for (size_t i = 0; i < 10; ++i) {
        ...
        ImGui::ImageButton((void*)(intptr_t)srvUavMemory.GetGPUHandle(descCounter).ptr, ImVec2(iconSize, iconSize), ImVec2(0, 0), ImVec2(1, 1), 0); // Замени на иконки
        ApplyGradientAndStarsEffect(ImVec2(iconSize, iconSize), baseColors[currentColorIndex], endColors[currentColorIndex]);
        currentColorIndex++;
        if (ImGui::IsItemHovered()) ImGui::SetTooltip("Слот %zu", i + 1);
        if (i < 9) ImGui::SameLine();
...
void UILayer::DrawRightTopBar()
{
    ...
    // 5.1 Миникарта
    ImGui::ImageButton((void*)(intptr_t)srvUavMemory.GetGPUHandle(descCounter).ptr, ImVec2(180.0f, 60.0f), ImVec2(0, 0), ImVec2(1, 1), 0); // Замени на текстуру миникарты
    ApplyGradientAndStarsEffect(ImVec2(iconSize, iconSize), baseColors[currentColorIndex], endColors[currentColorIndex]);
    currentColorIndex++;
    // 5.2 Текущее время
