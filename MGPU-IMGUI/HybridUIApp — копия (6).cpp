...
void HybridUIApp::Draw(const GameTimer& gt)
{
    ...
    if (IsUseSharedUI)
    {
        ...
        auto fenceOneStart = std::chrono::high_resolution_clock::now();
        ...
        {   
            ...
            std::chrono::duration<double> durationn = fenceEnd - fenceOneStart;
            ...
            auto starR = std::chrono::high_resolution_clock::now();


            UIPath->RenderEffects(secondDevice->GetCommandQueue(GQueueType::Compute));

            auto uiEndRE = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> durationn22135w = uiEndRE - starR;

            UIPath->Render(cmdList);

            auto uiEndRR = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> durationn22135 = uiEndRR - uiEndRE;

            auto uiEndR = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> durationn221356 = uiEndR - starR;
            ...
            std::chrono::duration<double> duration4 = end33 - start111;

            f = 1;

            auto uiEnd = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> durationn2213 = uiEnd - uistart;

            f = 1;
        }

        fenceOneStart = std::chrono::high_resolution_clock::now();
        ...
        if (currentFrameResource->PrimeCopyFenceValue == 0 || copyPrimeQueue->IsFinish(
            currentFrameResource->PrimeCopyFenceValue))
        {
            auto fencend = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> durationn44 = fencend - fenceOneStart;

            auto start1 = std::chrono::high_resolution_clock::now();

            auto cmdList = copyPrimeQueue->GetCommandList();

            cmdList->CopyResource(primeDeviceUITexture, secondDeviceUITexture);

            currentFrameResource->PrimeCopyFenceValue = copyPrimeQueue->ExecuteCommandList(cmdList);

            auto end1 = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> duration2 = end1 - start1;

            auto f = 1;
        }

        auto end2 = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> duration3 = end2 - overallStart;
    ...
    {
        auto ààStart = std::chrono::high_resolution_clock::now();

        UIPath->RenderEffects(primeDevice->GetCommandQueue(GQueueType::Compute));
        UIPath->Render(primeCmdList);

        auto end2231 = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> duration3 = end2231 - ààStart;
