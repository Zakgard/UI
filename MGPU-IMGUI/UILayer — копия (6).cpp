...
void UILayer::DrawBottomPanel()
{
    ...
    for (size_t i = 0; i < 15; ++i) {
        ...
        ImGui::ImageButton((void*)uiSRVs[skillCounter].GetGPUHandle().ptr, ImVec2(iconSize, iconSize), ImVec2(0, 0), ImVec2(1, 1), 0); // Замени на иконки
        //ApplyGradientAndStarsEffect(ImVec2(iconSize, iconSize), baseColors[currentColorIndex], endColors[currentColorIndex]);
        currentColorIndex++;
    ...
    for (size_t i = 0; i < 15; ++i) {
        ImGui::PushID(static_cast<int>(i));
        ImGui::ImageButton((void*)uiSRVs[skillCounter].GetGPUHandle().ptr, ImVec2(iconSize, iconSize), ImVec2(0, 0), ImVec2(1, 1), 0);
        //ApplyGradientAndStarsEffect(ImVec2(iconSize, iconSize), baseColors[currentColorIndex], endColors[currentColorIndex]);
        currentColorIndex++;
        if (ImGui::IsItemHovered()) ImGui::SetTooltip("Способность %zu", i + 1);
        if (i < 14) ImGui::SameLine();
...
void UILayer::Update()
{
    ...
    cmd->TransitionBarrier(uiTexs[uiDescriptorCounter].GetD3D12Resource(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    cmd->FlushResourceBarriers();

    cmd->SetDescriptorsHeap(&uiUAVs[uiDescriptorCounter]);
    cmd->SetPipelineState(uiPSOs[uiDescriptorCounter]);
    
   // cmdList->SetRootConstantBufferView(0, params);
    cmd->SetRoot32BitConstants(0, sizeof(GradientNoiseCB) / sizeof(float), &par, 0);
    cmd->SetRootDescriptorTable(1, &uiUAVs[uiDescriptorCounter]);
    ...
    cmd->TransitionBarrier(uiTexs[uiDescriptorCounter].GetD3D12Resource(), D3D12_RESOURCE_STATE_COMMON);
    cmd->FlushResourceBarriers();
...
void UILayer::InitializeGradientResources()
{
    ...
    uiPSOs[uiDescriptorCounter].SetShader(&gradientShader);
