...
static UINT skillCurentIndex = 0;
static UINT gradientCurrentIndex = 0;
std::vector<UINT> skillIndexes;
std::vector<UINT> navigationIndexes;
std::vector<UINT> gradientIndexes;
UINT hbBarIndex;
static UINT elementsCount = 0;
static UINT descCounter = 0;
...
{
    srvUavMemory = device->AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 250);
...
void UILayer::SetStyle()
{
    ...
    //  io.DisplaySize = ImVec2(1920, 1080); // 8K разрешение
...
void UILayer::DrawLeftBar()
{
    ...
    ImGui::ImageButton((void*)(intptr_t)srvUavMemory.GetGPUHandle(descCounter).ptr, ImVec2(buttonSize, buttonSize), ImVec2(0, 0), ImVec2(1, 1), 0); // Замени на реальную текстуру
    ...
    for (uint16_t i = 0; i < 5; i++) {
        ImGui::ImageButton((void*)(intptr_t)srvUavMemory.GetGPUHandle(descCounter).ptr, ImVec2(buttonSize, buttonSize), ImVec2(0, 0), ImVec2(1, 1), 0); // Замени на иконки
        ApplyGradientAndStarsEffect(ImVec2(iconSize, iconSize), baseColors[currentColorIndex], endColors[currentColorIndex]);
        currentColorIndex++;
...
void UILayer::DrawRightBar()
{
    ...
    for (size_t i = 0; i < 10; ++i) {
        ...
        ImGui::Image((void*)(intptr_t)srvUavMemory.GetGPUHandle(skillIndexes[skillCurentIndex]).ptr,
        ...
        ImGui::ImageButton((void*)srvUavMemory.GetGPUHandle(gradientIndexes[gradientCurrentIndex]).ptr,
            ImVec2(iconSize, iconSize),
            ImVec2(0, 0),
            ImVec2(1, 1),
            0);
        gradientCurrentIndex++;
        descCounter--;
        if (ImGui::IsItemHovered()) ImGui::SetTooltip("Слот %zu", i + 1);
        if (i < 9) ImGui::SameLine();
        ImGui::PopID();
...
void UILayer::DrawRightTopBar()
{
    ...
    // 5.1 Миникарта
    ImGui::ImageButton((void*)(intptr_t)srvUavMemory.GetGPUHandle(0).ptr, ImVec2(180.0f, 60.0f), ImVec2(0, 0), ImVec2(1, 1), 0); // Замени на текстуру миникарты
    ApplyGradientAndStarsEffect(ImVec2(iconSize, iconSize), baseColors[currentColorIndex], endColors[currentColorIndex]);
    currentColorIndex++;
    // 5.2 Текущее время
...
{
    ImGui::SetNextWindowPos(ImVec2(displaySize.x - iconSize - 40, displaySize.y - iconSize * 16));
    ImGui::SetNextWindowSize(ImVec2((iconSize + 18) * 1, iconSize * 10 + 150));
    ...
    //    descCounter--;
        // Кнопка поверх изображения (с отступом или без)
     //   ImGui::SetCursorPosX(ImGui::GetCursorPosX() + iconSize + 4.0f);
        ImGui::SetCursorPosY(ImGui::GetCursorPosY() - iconSize - 4.0f); // Смещаем назад на высоту изображения
        ImGui::ImageButton((void*)srvUavMemory.GetGPUHandle(gradientIndexes[gradientCurrentIndex]).ptr,
            ImVec2(iconSize - 7.0f, iconSize - 6.0f),
            ImVec2(0, 0));
        gradientCurrentIndex++;
    //    descCounter--;
...
void UILayer::DrawBottomPanel()
{
    ...
    ImGui::Image((void*)srvUavMemory.GetGPUHandle(hbBarIndex).ptr, ImVec2(750, 100), ImVec2(0, 0), ImVec2(1, 1));
...
void UILayer::Update(const std::shared_ptr<GCommandList>& cmdList)
{
    ...
    cmdList->SetRootDescriptorTable(1, &srvUavMemory, gradientIndexes[gradientCurrentIndex]);
...
void UILayer::InitializeGradientResources()
{
   ...
   //   uiUAVs[uiDescriptorCounter] = device->AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, globalCountFrameResources);
    ...
    gradientText.Layout = D3D12_TEXTURE_LAYOUT_64KB_UNDEFINED_SWIZZLE;
    ...
    uiTexs[uiDescriptorCounter].CreateUnorderedAccessView(&uavDesc, &srvUavMemory, descCounter);
    //gradientIndexes.push_back(descCounter);
    descCounter++;
    ...
    uiTexs[uiDescriptorCounter].CreateShaderResourceView(&srvDesc, &srvUavMemory, descCounter);
    gradientIndexes.push_back(descCounter);
    descCounter++;
...
void UILayer::InitializeHPBar()
{
   ...
   //   hbBarUAV = device->AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, globalCountFrameResources);
    ...
    hbBarTex.CreateUnorderedAccessView(&uavDesc, &srvUavMemory, descCounter);
    hbBarIndex = descCounter;
    descCounter++;
    ...
    hbBarTex.CreateShaderResourceView(&srvDesc, &srvUavMemory, descCounter);
    descCounter++;
...
void UILayer::UpdateHBBar(const std::shared_ptr<GCommandList>& cmdList, float value = 0.2f)
{
    ...
    cmdList->SetRootDescriptorTable(1, &srvUavMemory, hbBarIndex);
...
void UILayer::RenderEffects(const std::shared_ptr<GCommandQueue>& queue)
{
    ...
    skillCurentIndex = 0;
    gradientCurrentIndex = 0;
    const auto cmdList = queue->GetCommandList();

    cmdList->SetDescriptorsHeap(&srvUavMemory);
...
void UILayer::SetTexture()
{
    ...
    for (uint16_t i = 0; i < 62; i++)
    {
       ...
       //  skillPicturesDescriptors[i] = GDescriptor();
        loader.get()->LoadTextureFromFile(SKILL_PICURES_PLACEMENTS[i], device->GetDXDevice().Get(), srvUavMemory.GetCPUHandle(descCounter), &my_textures[i], &pictureSizeX, &pictureSizeY);
        skillIndexes.push_back(descCounter);
        descCounter++;
    }
    ...
    for (uint16_t i = 0; i < 1; i++)
    {
      ...
      //   uiNavigationPicturesDescriptors[i] = device->AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, globalCountFrameResources);
        ...
        loader.get()->LoadTextureFromFile(NAVIGATION_ICONS_PLACEMENTS[i], device->GetDXDevice().Get(), srvUavMemory.GetCPUHandle(descCounter), &my_textures[i], &pictureSizeX, &pictureSizeY);
        navigationIndexes.push_back(descCounter);
        descCounter++;
    }
