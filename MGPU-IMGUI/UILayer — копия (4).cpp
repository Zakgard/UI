...
void UILayer::DrawBottomPanel()
{
    ...
    for (size_t i = 0; i < 15; ++i) {
        ImGui::PushID(static_cast<int>(i));
        ImGui::ImageButton((void*)(intptr_t)skillPicturesDescriptors[skillCounter].GetGPUHandle().ptr, ImVec2(iconSize, iconSize), ImVec2(0, 0), ImVec2(1, 1), 0);
        ApplyGradientAndStarsEffect(ImVec2(iconSize, iconSize), baseColors[currentColorIndex], endColors[currentColorIndex]);
        currentColorIndex++;
        if (ImGui::IsItemHovered()) ImGui::SetTooltip("Способность %zu", i + 1);
        if (i < 14) ImGui::SameLine();
...
void UILayer::InitializeGradientResources(const std::shared_ptr<GDevice>& device)
{
    const std::wstring adr = L"Pictures\\Icons\\Chest.png";
    auto bricksTex = GTexture::LoadTextureFromFile(adr, device->GetCommandQueue()->GetCommandList());
    bricksTex->SetName(L"skillTex");
    assets->AddTexture(bricksTex);

    srvGradient = device->AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    ...
    gradientSignature.AddConstantParameter(sizeof(GradientNoiseCB) / sizeof(float), 0, 0);
    gradientSignature.AddDescriptorParameter(&textures[0], 1);
    gradientSignature.Initialize(device);

    GShader s(L"Shaders/GradientCompute.hsls", ComputeShader);
    s.LoadAndCompile();

    ComputePSO pso;

    pso.SetRootSignature(gradientSignature);
    pso.SetShader(&s);
    pso.Initialize(device);

    GRootSignature rootSign;
    rootSign.AddConstantParameter(sizeof(GradientNoiseCB) / sizeof(float), 0, 0);
    rootSign.AddDescriptorParameter(&textures[0], 1);
